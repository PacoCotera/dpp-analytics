...
def home_payload(connect, decorate_products, marketplace: str) -> dict:
    with connect() as conn, conn.cursor() as cur:
        ...
        if cutoff:
            ...
            series = _all(cur, "SELECT business_date,sales,orders,units,aov FROM mart.business_daily WHERE marketplace_id=%s AND reconciled_daily_report AND business_date BETWEEN least(date_trunc('year',%s::date)::date,%s::date-89) AND %s::date ORDER BY business_date", (marketplace, cutoff, cutoff, cutoff))
