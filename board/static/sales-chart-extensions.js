/* Sales chart extensions v2
   Adds FULL history, weekday-aware weekly run rate, weekend rhythm cues,
   and one canonical visual treatment for run-rate projections and calendar boundaries. */
(() => {
  'use strict';
  ...
  function ensureFullButton(){
    ...
    if(!range.querySelector('[data-range="full"]')){
      ...
      button.type='button';button.dataset.range='full';button.setAttribute('aria-pressed','false');button.textContent='FULL';
  ...
  function ensureTip(host){
    ...
    if(!tip){
      ...
      tip=document.createElement('div');tip.className='dpp-chart-tooltip home-week-tooltip sales-period-tooltip';tip.setAttribute('role','status');host.appendChild(tip);
    }
    return tip;
  }
  ...
  function showTip(host,tip,target,title,rows,footer){
    ...
    let left;
    if(placeRight){left=Math.min(targetRect.right-hostRect.left+gap,hostRect.width-tipW-8);tip.style.transform='translate(0,-50%)';}
    else{left=Math.max(targetRect.left-hostRect.left-gap,tipW+8);tip.style.transform='translate(-100%,-50%)';}
  ...
  function shell(label){
    ...
    const compact=window.innerWidth<=720,width=compact?520:960,height=340;
  ...
  function makeRunRatePattern(svg,id){
    const pattern=svg.append('defs').append('pattern').attr('id',id).attr('width',7).attr('height',7).attr('patternUnits','userSpaceOnUse').attr('patternTransform','rotate(45)');
    pattern.append('rect').attr('width',7).attr('height',7).attr('fill',RUNRATE_BG).attr('opacity',.48);
  ...
  function normalizeRunRateVisuals(){
    const svg=d3.select('#monthChart');
    ['#sales-run-rate-v5','#sales-full-run-rate','#sales-week-runrate-pattern'].forEach(id=>{
      const pattern=svg.select(id);if(pattern.empty())return;
      pattern.select('rect').attr('fill',RUNRATE_BG).attr('opacity',.48);
      pattern.select('line').attr('stroke',CURRENT_BAR).attr('stroke-width',1.7).attr('opacity',.48);
    });
    svg.selectAll('.dpp-ghost-bar,.sales-full-ghost,.sales-week-runrate').attr('stroke',CURRENT_BAR).attr('stroke-width',1).attr('stroke-opacity',.72);
    svg.selectAll('.dpp-projection-value,.sales-full-runrate-label,.sales-week-runrate-label')
      ...
      .each(function(){
        ...
        if(value)node.text(`Run rate · ${value}`);
      });
  }

  function drawFull(){
    const rows=DATA?.months_full||[];
    const data=rows.map(row=>({...row,key:String(row.month||'').slice(0,7),date:parseDate(`${String(row.month||'').slice(0,7)}-01`),value:Number(row.sales||0),orders:Number(row.orders||0),units:Number(row.units||0),projected:row.partial?Number(DATA?.headline?.projected_month_sales||0):0}))
    ...
    const patternId='sales-full-run-rate';makeRunRatePattern(ctx.svg,patternId);
    const bars=ctx.plot.selectAll('.dpp-bar.sales-full').data(data).join('rect').attr('class','dpp-bar sales-full')
      .attr('x',d=>x(d.key)).attr('width',x.bandwidth()).attr('y',d=>y(d.value)).attr('height',d=>Math.max(1,ctx.innerH-y(d.value))).attr('rx',4).attr('fill',d=>d.partial?CURRENT_BAR:HISTORICAL_BAR);
    const projected=data.filter(d=>d.partial&&d.projected>d.value);
    ctx.plot.selectAll('.sales-full-ghost').data(projected).join('rect').attr('class','sales-full-ghost')
      .attr('x',d=>x(d.key)).attr('width',x.bandwidth()).attr('y',d=>y(d.projected)).attr('height',d=>Math.max(1,y(d.value)-y(d.projected))).attr('rx',4)
      .attr('fill',`url(#${patternId})`).attr('stroke',CURRENT_BAR).attr('stroke-width',1).attr('stroke-opacity',.72);
    ...
    byYear.forEach(([year,items],index)=>{
      const first=items[0],last=items[items.length-1],left=x(first.key),right=x(last.key)+x.bandwidth(),center=(left+right)/2;
      if(index>0){
        const divider=left-(x.step()-x.bandwidth())/2;
        yearLayer.append('line').attr('x1',divider).attr('x2',divider).attr('y1',18).attr('y2',ctx.innerH).attr('stroke','#aaa197').attr('stroke-width',1).attr('stroke-dasharray','3 4').attr('opacity',.76);
      }
      yearLayer.append('text').attr('x',center).attr('y',11).attr('text-anchor','middle').attr('fill',MUTED).attr('font-size',8.7).attr('font-weight',760).attr('letter-spacing','.06em').text(String(year));
    });

    if(!ctx.compact&&data.length<=16){
      ctx.plot.selectAll('.sales-full-value').data(data.filter(d=>!d.partial)).join('text').attr('class','dpp-value sales-full-value')
        .attr('x',d=>x(d.key)+x.bandwidth()/2).attr('y',d=>y(d.value)-8).attr('text-anchor','middle').text(d=>shortMoney(d.value));
    }
    ctx.plot.selectAll('.sales-full-runrate-label').data(projected).join('text').attr('class','sales-full-runrate-label')
      .attr('x',d=>x(d.key)+x.bandwidth()/2).attr('y',d=>Math.max(22,y(d.projected)-8)).attr('text-anchor','middle').text(d=>`Run rate · ${shortMoney(d.projected)}`);

    let ticks=data;if(data.length>18)ticks=data.filter((d,i)=>d.date.getUTCMonth()%3===0||i===0||i===data.length-1);
    ...
    bars.attr('tabindex',0).on('pointerenter pointermove focus',function(event,d){showTip(ctx.host,ctx.tip,this,d3.utcFormat('%B %Y')(d.date),[{label:'Sales',value:money(d.value)}],`${nf.format(d.orders)} orders · ${nf.format(d.units)} units`);}).on('pointerleave blur',()=>hideTip(ctx.tip));
  }
  ...
  function weeksFromRows(rows){
    ...
    return d3.rollups(daily,values=>{
      const daySales=Array(7).fill(0);values.forEach(v=>{const idx=(v.date.getUTCDay()+6)%7;daySales[idx]+=v.sales;});
      return {sales:d3.sum(values,d=>d.sales),days:values.length,dates:values.map(d=>d.date).sort(d3.ascending),daySales};
    },d=>+d3.utcMonday.floor(d.date)).map(([week,value])=>({week:new Date(Number(week)),...value})).sort((a,b)=>d3.ascending(a.week,b.week));
  }

  function currentWeekProjection(){
    const weeks=weeksFromRows((DATA?.series||[]).slice(-90));
    const current=weeks[weeks.length-1];if(!current||current.days<=0||current.days>=7||current.sales<=0)return null;
    const prior=weeks.slice(0,-1).filter(w=>w.days>=7&&w.sales>0).slice(-8);if(!prior.length)return null;
    const latest=current.dates[current.dates.length-1],elapsedIndex=(latest.getUTCDay()+6)%7;
    const shares=prior.map(w=>d3.sum(w.daySales.slice(0,elapsedIndex+1))/w.sales).filter(v=>Number.isFinite(v)&&v>.04&&v<=1);
    const baseline=d3.mean(prior.slice(-4),w=>w.sales)||0,historicalShare=d3.mean(shares);
  ...
  function correctWeeklyRunRate(){
    const svg=d3.select('#monthChart');
    svg.selectAll('.sales-week-runrate,.sales-week-runrate-label').remove();svg.select('defs#sales-week-runrate-defs').remove();
    if(activeRange()!=='90d'||!DATA)return;
    const result=currentWeekProjection();if(!result)return;
    const {current,latest,projected}=result;if(projected<=current.sales)return;

    const grouped=weeksFromRows((DATA.series||[]).slice(-90)).slice(-13);
    const compact=window.innerWidth<=720,width=compact?520:960;
    const m={top:28,right:compact?14:58,bottom:44,left:compact?54:62},innerW=width-m.left-m.right,innerH=340-m.top-m.bottom;
    ...
    plot.append('rect').attr('class','sales-week-runrate').attr('pointer-events','none')
      .attr('x',x(+current.week)).attr('width',x.bandwidth()).attr('y',projectedY).attr('height',Math.max(1,actualY-projectedY)).attr('rx',4)
      .attr('fill',`url(#${patternId})`).attr('stroke',CURRENT_BAR).attr('stroke-width',1).attr('stroke-opacity',.72);
    plot.append('text').attr('class','sales-week-runrate-label').attr('pointer-events','none')
      .attr('x',x(+current.week)+x.bandwidth()/2).attr('y',Math.max(11,projectedY-8)).attr('text-anchor','middle').text(`Run rate · ${shortMoney(projected)}`);
  ...
  function tintWeekends(){
    ...
    d3.select('#monthChart').selectAll('.dpp-bar').attr('fill',d=>{
      const date=d?.date;if(!(date instanceof Date))return HISTORICAL_BAR;
      const day=date.getUTCDay();return day===0||day===6?WEEKEND_BAR:HISTORICAL_BAR;
    });
  }

  function accentMonthBoundary(){
    if(activeRange()!=='28d')return;
    const svg=d3.select('#monthChart');
  ...
  function updateFullRail(){
    const rail=document.getElementById('salesChartKpiRail'),rows=DATA?.months_full||[];
    if(!rail||!rows.length)return;
    const sales=sum(rows,'sales'),orders=sum(rows,'orders'),units=sum(rows,'units');
    const completed=rows.filter(row=>!row.partial),avgMonth=completed.length?sum(completed,'sales')/completed.length:0;
    let benchmark=null;
    if(completed.length>=24){const recent=sum(completed.slice(-12),'sales'),prior=sum(completed.slice(-24,-12),'sales');if(prior>0)benchmark=100*(recent-prior)/prior;}
    const tone=benchmark==null?'':benchmark>0?'good':benchmark<0?'bad':'';
    rail.dataset.range='full';
    rail.innerHTML=`<div class="sales-chart-kpi"><div class="sales-chart-kpi-label">Full sales</div><div class="sales-chart-kpi-value">${money(sales)}</div><div class="sales-chart-kpi-note">reconciled history</div></div>
  ...
  function renderExtensions(){
    ...
    if(range==='full'){drawFull();updateFullRail();normalizeRunRateVisuals();return;}
    if(range==='90d'){correctWeeklyRunRate();normalizeRunRateVisuals();return;}
    if(range==='28d'){tintWeekends();accentMonthBoundary();return;}
    normalizeRunRateVisuals();
  }
  ...
  function bind(){
    ...
    range.addEventListener('click',event=>{
      ...
      setTimeout(renderExtensions,100);setTimeout(renderExtensions,240);
    });
    return true;
  }
  ...
  function init(){
    let attempts=0;
    const ready=setInterval(()=>{attempts+=1;if(ensureFullButton()&&bind()||attempts>50)clearInterval(ready);},50);
    fetch('/api/sales',{cache:'no-store'})
      .then(response=>response.ok?response.json():Promise.reject(new Error(`HTTP ${response.status}`)))
      .then(data=>{DATA=data;setTimeout(renderExtensions,260);setTimeout(renderExtensions,640);})
      .catch(()=>{});
    let timer;window.addEventListener('resize',()=>{clearTimeout(timer);timer=setTimeout(renderExtensions,260);},{passive:true});
  }
