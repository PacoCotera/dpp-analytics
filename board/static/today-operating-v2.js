...
(() => {
  ...
  function compactStateLabels(){
    ...
    Object.entries(map).forEach(([id,label])=>{const node=document.getElementById(id);if(node)node.textContent=label;});
  }
  ...
  function compactDatePicker(){
    ...
    buttons.forEach((button,index)=>{
      ...
      if(label&&index>1){const raw=label.textContent.trim();if(raw.length>3)label.textContent=raw.slice(0,3);}
      if(sub){const raw=sub.textContent.trim();sub.textContent=raw.replace(/closed/i,'').replace(/\s+/g,' ').trim();}
    });
  }
  ...
  const refresh=()=>{compactStateLabels();compactDatePicker();syncSelectedLanguage();};
  refresh();
...
/* Today showcase v2: inline cockpit runtime; no separately served assets required. */
(() => {
  'use strict';
  if(!document.body.classList.contains('today-shell'))return;
  ...
  function compactPicker(){
    [...document.querySelectorAll('#dayPicker .day-choice')].forEach(btn=>{
      ...
      const label=btn.querySelector('b');if(label)label.textContent=d3.utcFormat('%a')(d).slice(0,1);
  ...
  function restructureRail(){
    ...
    if(!grid){grid=document.createElement('div');grid.className='today-kpi-grid';const rows=[...rail.querySelectorAll('.today-state-row')];rail.querySelector('.today-pulse-title').after(grid);rows.forEach(r=>grid.appendChild(r));}
  ...
  function renderInventory(){
    ...
    const a=document.getElementById('todayInvAvailable'),c=document.getElementById('todayInvCover'),r=document.getElementById('todayInvActions'),n=document.getElementById('todayInvActionNote'),box=document.getElementById('todayInvAttention');
    if(a)a.textContent=new Intl.NumberFormat('en-US').format(Number(s.available||0));
    if(c)c.textContent=s.portfolio_days_cover!=null?`${Number(s.portfolio_days_cover).toFixed(1)} days portfolio cover`:'portfolio cover unavailable';
    if(r)r.textContent=String(Number(s.needs_action||0));
    if(n)n.textContent=Number(s.stockouts||0)>0?`${s.stockouts} stockout · ${s.produce||0} produce`:`${s.produce||0} produce · ${s.plan||0} plan`;
    if(box)box.classList.toggle('attention',Number(s.needs_action||0)>0);
  }

  function selectedRows(){
    const rows=(TODAY?.recent_daily||[]).map(r=>({...r,date:parseDate(r.business_date),sales:Number(r.sales||0),orders:Number(r.orders||0),units:Number(r.units||0)})).filter(r=>r.date);
    const p=period();if(p==='7')return rows.slice(-7);if(p==='mtd'){const target=String(TODAY?.selected_date||rows.at(-1)?.business_date||'').slice(0,7);return rows.filter(r=>String(r.business_date).slice(0,7)===target)}return rows.slice(-30);
  }
  ...
  function renderRhythmRail(rows){
    const rail=ensureRhythmRail();if(!rail)return;
    const sales=sum(rows,'sales'),orders=sum(rows,'orders'),units=sum(rows,'units'),days=Math.max(1,rows.length),pace=sales/days;let bench=null,note='';const p=period(),all=(TODAY?.recent_daily||[]);
    if(p==='7'&&all.length>=14){const prior=sum(all.slice(-14,-7),'sales');if(prior>0)bench=100*(sales-prior)/prior;note='vs prior 7D'}else if(p==='mtd'){bench=TODAY?.context?.mtd_delta_pct;note='vs same days last month'}else{bench=TODAY?.context?.last30_delta_pct;note='vs prior 30D'}
    const cls=bench>0?'good':bench<0?'bad':'';
    rail.innerHTML=`<div class="today-rhythm-kpi"><div class="label">Sales</div><strong>${money(sales)}</strong><small>${p==='mtd'?'month to date':p==='7'?'last 7 days':'last 30 days'}</small></div><div class="today-rhythm-kpi"><div class="label">Orders · units</div><strong>${orders} · ${units}</strong><small>volume in period</small></div><div class="today-rhythm-kpi"><div class="label">Daily pace</div><strong>${money(pace)}</strong><small>average per day</small></div><div class="today-rhythm-kpi"><div class="label">Benchmark</div><strong class="${cls}">${bench==null?'—':pct(bench)}</strong><small>${note||'comparison unavailable'}</small></div>`;
  }
  ...
  function drawRhythm(){
    ...
    const host=svg.node().parentElement;let tip=host.querySelector('.today-rhythm-tip');if(!tip){tip=document.createElement('div');tip.className='dpp-chart-tooltip today-rhythm-tip';host.appendChild(tip)}
    bars.attr('tabindex',0).on('pointerenter pointermove focus',function(ev,r){tip.innerHTML=`<strong>${d3.utcFormat('%a, %b %-d')(r.date)}</strong><span>Sales ${money(r.sales)}</span><span>${r.orders} orders · ${r.units} units${r.business_date===selected?' · selected':''}</span>`;const hr=host.getBoundingClientRect(),br=this.getBoundingClientRect();tip.style.left=`${Math.min(hr.width-100,Math.max(100,br.left-hr.left+br.width/2))}px`;tip.style.top=`${Math.max(64,br.top-hr.top+10)}px`;tip.classList.add('show')}).on('pointerleave blur',()=>tip.classList.remove('show'));
  ...
  async function restoreVersionFooter(){
    const footer=document.querySelector('.live-footer');if(!footer||footer.querySelector('.today-version'))return;
    try{const html=await fetch('/',{cache:'no-store'}).then(r=>r.text());const m=html.match(/main\s+([0-9a-f]{8})/i);if(!m)return;const s=document.createElement('span');s.className='today-version';s.textContent=`Dirty Pawz Press Analytics · main ${m[1]}`;footer.appendChild(s)}catch(_){ }
  }

  async function refreshData(){
    try{const q=activeDate()&&activeDate()!==document.querySelector('#dayPicker .day-choice.live')?.dataset.date?`?date=${encodeURIComponent(activeDate())}`:'';const [t,i]=await Promise.all([fetch('/api/today'+q,{cache:'no-store'}).then(r=>r.ok?r.json():null),fetch('/api/inventory',{cache:'no-store'}).then(r=>r.ok?r.json():null)]);if(t)TODAY=t;if(i)INVENTORY=i;compactPicker();restructureRail();renderInventory();drawRhythm();restoreVersionFooter()}catch(_){ }
  }
