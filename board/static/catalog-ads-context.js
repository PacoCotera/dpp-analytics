...
function familyPaidRead(family) {
  ...
  const maturity = String(
    family.ad_attribution_state || "UNAVAILABLE",
  ).toLowerCase();
...
function productPaidRead(product) {
  ...
  if (!spend) return "";
  const maturity = String(
    product.ad_attribution_state || "UNAVAILABLE",
  ).toLowerCase();
...
function decorate(payload) {
  ...
  document.querySelectorAll(".family[data-family]").forEach((row) => {
    ...
    if (!family) return;
    appendOnce(
      row.querySelector(".family-meta"),
      familyPaidRead(family),
      "adsRead",
    );
  });
  ...
  document
    .querySelectorAll(
      'a.child[href^="/product?sku="], a.analysis-link[href^="/product?sku="]',
    )
    .forEach((row) => {
      ...
      if (!product) return;
      const meta =
        row.querySelector(".child-meta") ||
        row.querySelector(".analysis-identity span");
...
function attach(payload) {
  ...
  const ready = document
    .getElementById("portfolioBasis")
    ?.textContent?.includes("28D through");
  if (!ready) {
