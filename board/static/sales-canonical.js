...
(() => {
  ...
  const shortMoney = (v) => {
    const n = Number(v || 0),
      a = Math.abs(n);
    if (a >= 1e6)
      return `${n < 0 ? "−" : ""}$${(a / 1e6).toFixed(a >= 1e7 ? 0 : 1)}m`;
    if (a >= 1e3)
      return `${n < 0 ? "−" : ""}$${(a / 1e3).toFixed(a >= 1e4 ? 0 : 1)}k`;
  ...
  const parseDate = (s) =>
    s ? new Date(`${String(s).slice(0, 10)}T12:00:00Z`) : null;
  ...
  function orderStatus(status) {
    ...
    if (key === "PENDING_AVAILABILITY")
      return "Amazon processing · availability";
  ...
  function orderStatusTone(status) {
    const key = String(status || "")
      ...
      .toUpperCase();
    if (
      ["PENDING", "PENDING_AVAILABILITY", "INVOICE_UNCONFIRMED"].includes(key)
    )
      return "waiting";
    if (["SHIPPED", "UNSHIPPED", "PARTIALLY_SHIPPED"].includes(key))
      return "active";
    if (["CANCELLED", "CANCELED", "UNFULFILLABLE"].includes(key))
      return "problem";
    return "neutral";
  }
  ...
  function renderSignals() {
    ...
    set(
      "mtdVolume",
      `${nf.format(h.orders_mtd || 0)} orders · ${nf.format(h.units_mtd || 0)} units`,
    );
    set(
      "mtdNote",
      `${pct(h.delta_mtd_pct)} vs same days last month`,
      cls(h.delta_mtd_pct),
    );
    ...
    set(
      "t28Volume",
      `${nf.format(h.orders_t28 || 0)} orders · ${nf.format(h.units_t28 || 0)} units`,
    );
    set("t28Note", `${pct(h.delta28_pct)} vs prior 28`, cls(h.delta28_pct));
    set("todaySales", money(t.sales_today));
    set(
      "todayMeta",
      `${t.orders_today || 0} orders · ${t.units_today || 0} units`,
    );
    ...
    set(
      "todayPace",
      ord < 3
        ? "Pace is still low-signal"
        : `${pct(pv)} vs same weekday / same time`,
      ord < 3 ? "" : cls(pv),
    );
    ...
    set(
      "ytdVolume",
      `${nf.format(h.orders_ytd || 0)} orders · ${nf.format(h.units_ytd || 0)} units`,
    );
  ...
  function renderProductRead() {
    ...
    set(
      "productChangeState",
      change > 0 ? "Improving" : change < 0 ? "Weakening" : "Flat",
    );
    set(
      "productChangeDriver",
      mover.product
    ...
    set(
      ...
      concentration == null ? "—" : `${Number(concentration).toFixed(1)}%`,
    );
    ...
    set(
      "productBreadth",
      `${nf.format(growing)} ↑ / ${nf.format(declining)} ↓`,
    );
  ...
  function renderProducts() {
    ...
    document
      ...
      ?.classList.toggle("products-expanded", PRODUCTS_EXPANDED);
    if (!control) return;
    control.hidden = hiddenCount === 0;
    control.setAttribute("aria-expanded", PRODUCTS_EXPANDED ? "true" : "false");
    set(
      "productsMoreLabel",
      PRODUCTS_EXPANDED ? "Show leading products" : "Show all products",
    );
    set(
      "productsMoreCount",
      PRODUCTS_EXPANDED
        ? `${nf.format(rows.length)} shown`
        : `${nf.format(hiddenCount)} more`,
    );
  }
  function renderOrders() {
    ...
    out.innerHTML = rows
      .map((r, index) => {
        ...
        const unitCount = details.reduce(
          (total, item) =>
            total + Number(item.quantity_ordered ?? item.quantity ?? 0),
          0,
        );
        const items = details.length
          ? `<div class="sales-order-items__summary">${nf.format(details.length)} line item${details.length === 1 ? "" : "s"} · ${nf.format(unitCount)} unit${unitCount === 1 ? "" : "s"}</div>${details
              .map((item) => {
                ...
                const identity = [item.sku, item.asin]
                  .filter(Boolean)
                  .join(" · ");
      ...
      .join("");
    document
      .getElementById("orderEvidence")
      ?.classList.toggle("orders-expanded", ORDERS_EXPANDED);
    ...
    set(
      "ordersMoreLabel",
      ORDERS_EXPANDED ? "Show fewer orders" : "Show all recent orders",
    );
    set(
      "ordersMoreCount",
      ORDERS_EXPANDED
  ...
  function ensureTip(host) {
    ...
    if (!tip) {
      ...
      tip.className =
        "dpp-chart-tooltip home-week-tooltip sales-period-tooltip";
  ...
  function drawMonthly(rows, isFull = false) {
    ...
    const x = d3
      ...
      y = d3
        ...
        .domain([
          0,
          d3.max(data, (r) => Math.max(r.value, r.projection || 0)) || 1,
        ])
        .nice(4)
        .range([c.ih, 0]);
    grid(c, y);
    ...
    if (isFull && data.length > 18)
      ticks = data.filter(
        (r, i) =>
          r.date.getUTCMonth() % 3 === 0 || i === 0 || i === data.length - 1,
      );
    axis(
      c,
      x,
  ...
  function currentWeekProjection(weeks) {
    ...
    const latest = current.dates.at(-1),
      ...
      pace =
        hshare > 0
          ? current.sales / hshare
          : (current.sales / Math.max(current.days, 1)) * 7,
      confidence = Math.min(1, Math.max(1, current.days) / 7),
      projected = Math.max(
        current.sales,
        baseline * (1 - confidence) + pace * confidence,
      );
  ...
  function drawWeekly() {
    ...
    copy(
      "Weekly sales",
      `90 days · weekly sales${weeks.at(-1).days < 7 ? " · current week partial" : ""}`,
    );
    const c = shell("Ninety days of weekly sales");
    const ymax =
        d3.max(weeks, (w) => Math.max(w.sales, projection?.projected || 0)) ||
        1,
      x = d3
        .scaleBand()
    ...
    axis(
      ...
      x,
      weeks
        .filter((w, i) => i === 0 || i === weeks.length - 1 || i % 3 === 0)
        .map((w) => +w.week),
    ...
    weeks.forEach((w, i) => {
      ...
      const prior = weeks
        ...
        .slice(-4);
      w.benchmark =
        w.days >= 7 && prior.length === 4
          ? d3.mean(prior, (x) => x.sales)
          : null;
    });
    bars
      .attr("tabindex", 0)
        ...
        if (w.days >= 7) {
          if (w.previous?.days >= 7 && w.previous.sales > 0)
            tr.push({
              label: "LW",
              value: pct(
                (100 * (w.sales - w.previous.sales)) / w.previous.sales,
              ),
            });
          if (w.benchmark > 0)
            tr.push({
  ...
  function drawMonthBoundary(c, x, rows) {
    ...
    const prev = d3.utcMonth.offset(start, -1),
      ...
      closed = (DATA.months || []).find(
        (r) => String(r.month || "").slice(0, 7) === key && !r.partial,
      );
    if (closed)
      c.plot
        .append("text")
        ...
        .text(
          `${d3.utcFormat("%b")(prev).toUpperCase()} CLOSED · ${shortMoney(closed.sales)}`,
        );
  }
  function drawDaily() {
    const h = DATA.headline || {},
    ...
    const c = shell(
      "Twenty-eight days of daily sales with week and month boundaries",
    );
    const x = d3
        .scaleBand()
    ...
    axis(
      ...
      x,
      rows
        .filter((r, i) => i === 0 || i === rows.length - 1 || i % 7 === 0)
        .map((r) => +r.date),
      (v) => d3.utcFormat("%b %-d")(new Date(Number(v))),
    );
    rows.forEach((r, i) => {
