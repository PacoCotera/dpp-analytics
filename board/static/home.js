...
function stateRead(delta, actions) {
  const momentum = Number(delta || 0), decisionCount = Number(actions || 0);
  const decisionCopy = decisionCount ? `${decisionCount} operating decision${decisionCount === 1 ? ' needs' : 's need'} attention.` : '';
...
function renderAds(ads) {
  const a = ads || {}, metrics = document.getElementById('adsMetrics'), headline = document.getElementById('adsHeadline'), note = document.getElementById('adsNote');
  if (!a.through_date) {
    metrics.hidden = true;
    headline.textContent = 'Waiting for Amazon Ads access';
    note.textContent = a.note || 'Sales interpretation remains independent until advertising data is available.';
    return;
  }
  ...
  headline.textContent = a.trusted ? 'Advertising context is decision-grade' : 'Advertising context is still provisional';
...
function renderAttention(data, decisionCount) {
  ...
  const title=document.getElementById('attentionTitle'), copy=document.getElementById('attentionCopy'), container=document.getElementById('attention');
...
function renderDrivers(data,businessSales) {
  const movers=(data.movers||[]).slice(0,3), container=document.getElementById('movers');
...
function render(data) {
  ...
  document.getElementById('todaySales').textContent=money(today.sales_today); document.getElementById('todayNote').textContent=`incl. IVA · ${integer(today.orders_today)} orders · ${integer(today.units_today)} units`;
  document.getElementById('decisionCount').textContent=integer(decisionCount); document.getElementById('decisionNote').textContent=decisionCount===1?'decision needs attention':'decisions need attention'; document.getElementById('attentionCount').textContent=integer(decisionCount);
  renderAds(data.ads); renderAttention(data,decisionCount); renderDrivers(data,Number(rolling.sales_t28||0));
  if(window.DPPCharts?.homeRhythm){const cutoff=String(rolling.business_date||'').slice(0,10),reconciledSeries=(data.series||[]).filter(row=>!cutoff||String(row.business_date||'').slice(0,10)<=cutoff);window.DPPCharts.homeRhythm('#spark',reconciledSeries,data.weekly_products);}
}
