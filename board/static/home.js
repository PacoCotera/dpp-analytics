...
function renderAttention(data, decisionCount) {
  ...
  const copy = document.getElementById('attentionCopy');
  ...
  if (!attention.length) {
    ...
    container.innerHTML = '<div class="attention-clear"><strong>Operations are clear.</strong><p>Use the business rhythm and product drivers to understand performance; there is no immediate inventory action.</p></div>';
    return;
  }

  const item = attention[0];
  title.textContent = decisionCount === 1 ? 'One decision needs attention' : `${decisionCount} decisions need attention`;
  copy.textContent = item.action === 'STOCKOUT'
...
function renderDrivers(data, businessSales) {
  ...
  if (!movers.length) {
    container.innerHTML = '<div class="empty"><strong>No product-driver data yet.</strong>Drivers will appear as history builds.</div>';
    return;
  }

  container.innerHTML = movers.map((item, index) => {
    const deltaTone = tone(item.delta28_pct);
    const image = item.image_url
      ? `<img src="${escapeHtml(item.image_url)}" alt="" loading="lazy">`
      : '';
    const direction = Number(item.delta28_pct) >= 8
      ? 'accelerating'
...
function render(data) {
  ...
  const decisionCount = Number(inventory.needs_action || 0);
  const read = stateRead(rolling.delta28_pct, decisionCount);
