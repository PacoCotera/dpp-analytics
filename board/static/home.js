...
function renderAds(ads) {
  ...
  if (!a.through_date) {
    ...
    note.textContent =
      a.note ||
      "Sales interpretation remains independent until advertising data is available.";
    return;
  }
  metrics.hidden = false;
  document.getElementById("adsSpend").textContent = money(a.spend);
  document.getElementById("adsRoas").textContent =
    a.roas == null ? "—" : `${Number(a.roas).toFixed(2)}×`;
  document.getElementById("adsAcos").textContent =
    a.acos == null ? "—" : `${Number(a.acos).toFixed(1)}%`;
  document.getElementById("adsTacos").textContent =
    a.tacos == null ? "—" : `${Number(a.tacos).toFixed(1)}%`;
...
function renderAttention(data, decisionCount) {
  ...
  if (!attention.length) {
    ...
    copy.textContent =
      "No stockout, production or planning exception is currently flagged.";
    container.innerHTML =
  ...
  title.textContent =
    decisionCount === 1
      ? "One decision needs attention"
      : `${decisionCount} decisions need attention`;
  copy.textContent =
  ...
  container.innerHTML = movers
    .map((item, index) => {
      const deltaTone = tone(item.delta28_pct),
        ...
        share =
          businessSales > 0
            ? (100 * Number(item.sales_t28 || 0)) / businessSales
            : null,
...
function render(data) {
  ...
  document.getElementById("sales28Note").textContent =
    "incl. IVA · Sales & Traffic";
  ...
  document.getElementById("momentumNote").textContent =
    "vs prior 28 · same gross basis";
  ...
  if (window.DPPCharts?.homeRhythm) {
    const cutoff = String(rolling.business_date || "").slice(0, 10),
      reconciledSeries = (data.series || []).filter(
        (row) =>
          !cutoff || String(row.business_date || "").slice(0, 10) <= cutoff,
      );
    window.DPPCharts.homeRhythm(
      "#spark",
      reconciledSeries,
      data.weekly_products,
    );
  }
}
async function load() {
  try {
    ...
    document.getElementById("stateHeadline").textContent =
      "Operating feed unavailable";
