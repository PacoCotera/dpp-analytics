...
function renderDrivers(data, businessSales) {
  ...
  container.innerHTML = movers
    .map((item, index) => {
      ...
      const read = `${index === 0 ? 'Largest driver · ' : ''}${share == null ? 'share unavailable' : `${share.toFixed(0)}% of 28D shopper spend`} · ${direction} ${percent(item.delta28_pct)}`;
...
function render(data) {
  ...
  if (window.DPPCharts?.homeRhythm) {
    ...
    const reconciledSeries = (data.series || []).filter(
