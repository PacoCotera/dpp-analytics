...
const DOMAIN_DEFINITIONS = [
  { key: 'today', label: 'Today', affects: 'Live sales, orders and day-state decisions', terms: ['orders'], critical: true },
  { key: 'sales', label: 'Sales', affects: 'Historical sales, momentum and product contribution', terms: ['data_kiosk', 'data kiosk'], critical: true },
  { key: 'products', label: 'Products', affects: 'Catalog identity, offer state and product drill-down', terms: ['catalog', 'seller_listings', 'seller listings'], critical: true },
  { key: 'inventory', label: 'Inventory', affects: 'Stock position, cover and replenishment actions', terms: ['inventory'], critical: true },
  { key: 'finance', label: 'Finance', affects: 'Accounting periods, Amazon postings and contribution', terms: ['finance', 'settlement'], critical: true },
];
...
function buildDomains(payload) {
  ...
  const adsState = ads.state === 'HEALTHY' ? 'healthy' : ads.state === 'ATTENTION' ? 'degraded' : 'disconnected';
  core.push({
    key: 'ads',
    label: 'Ads',
...
function stateLabel(state) {
  return { healthy: 'Healthy', stale: 'Stale', degraded: 'Degraded', failed: 'Failed', disconnected: 'Not configured' }[state] || 'Unknown';
}
...
function renderJobs() {
  ...
  if (!rows.length && !expanded) {
    byId('jobs').innerHTML = '<div class="empty"><strong>No pipeline exceptions.</strong> Technical source detail is quiet because no tracked job needs attention.</div>';
    return;
  }
...
function render(payload) {
  ...
  if (blocked.length) {
    byId('healthTitle').textContent = `${blocked.length} decision domain${blocked.length === 1 ? ' is' : 's are'} not ready.`;
    byId('healthCopy').textContent = `${healthyCritical} of ${critical.length} decision-critical domains are healthy. ${blocked.map(domain => domain.label).join(', ')} should not be treated as decision-ready.`;
    byId('trustNote').textContent = 'Use healthy domains normally. Wait on the affected business surface until its required source recovers.';
  } else if (caution.length) {
    byId('healthTitle').textContent = 'Core decisions are available with caveats.';
    byId('healthCopy').textContent = `${healthyCritical} of ${critical.length} decision-critical domains are fully healthy; ${caution.map(domain => domain.label).join(', ')} ${caution.length === 1 ? 'needs' : 'need'} caution.`;
    byId('trustNote').textContent = 'The board is usable, but inspect the affected domain before making a time-sensitive decision.';
  } else {
    byId('healthTitle').textContent = 'Core seller decisions are supported.';
    byId('healthCopy').textContent = `All ${critical.length} decision-critical domains are healthy.${optionalUnavailable.length ? ` ${optionalUnavailable.map(domain => domain.label).join(', ')} is optional and currently unavailable.` : ''}`;
    byId('trustNote').textContent = 'Today remains provisional by design; historical Sales and Finance retain their own reconciliation and close-state semantics.';
  }
