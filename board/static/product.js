...
function renderHealth(payload) {
  ...
  const delta =
    performance.delta28_pct == null ? null : Number(performance.delta28_pct);
  const cover =
    profile.days_cover_with_inbound == null
      ? null
      : Number(profile.days_cover_with_inbound);
  ...
  if (!sellable) {
    ...
    reasons.push(
      "Demand interpretation is secondary until listing state is resolved.",
    );
    ...
    reasons.push(
      delta == null
        ? "Recent sales are active."
        : `${percent(delta)} sales versus the prior 28 days.`,
    );
  } else if (Number(traffic.sessions_t28 || 0) > 0) {
    headline = "Traffic is arriving, but units are not.";
    reasons.push(
      `${integer(traffic.sessions_t28)} sessions produced no recent units.`,
    );
  } else {
  ...
  if (cover != null)
    reasons.push(`${cover.toFixed(0)} days cover including inbound.`);
...
function renderHero(profile) {
  ...
  const actionTone =
    action === "OK"
      ? "good"
      : action === "STOCKOUT" || action === "PRODUCE"
        ? "bad"
        : "warn";
  const chips = [
    action
      ? `<span class="chip ${actionTone}">${escapeHtml(action)}</span>`
      : "",
    profile.listing_status
      ? `<span class="chip">${escapeHtml(profile.listing_status)}</span>`
      : "",
  ].join("");
...
function renderListingAndInventory(profile, commercial, ads) {
  ...
  byId("variationRead").textContent = attributeValues.length
    ...
    : commercial.product_role === "SELLABLE_STANDALONE"
      ? "Standalone"
      : "—";
  byId("variationNote").textContent = commercial.parent_asin
    ? "child variation"
    : "commercial offer";
  ...
  const hasAds = Boolean(
    ads.through_date && Number(ads.observed_ads_days || 0) > 0,
  );
  if (!hasAds) {
    ...
    byId("adsNote").textContent =
      `through ${String(ads.through_date).slice(5)}`;
  } else {
...
function renderMetrics(profile, performance, traffic, economics) {
  ...
  byId("delta28").textContent =
    performance.delta28_pct == null
      ? "no prior baseline"
      : `${percent(performance.delta28_pct)} vs prior`;
  byId("sessions28").textContent = integer(traffic.sessions_t28);
  byId("cvr28").textContent =
    traffic.cvr_t28 == null ? "—" : percent(traffic.cvr_t28, { sign: false });
  byId("stockRead").textContent =
    `${integer(profile.available)} + ${integer(profile.inbound)}`;
  byId("cover").textContent =
    profile.days_cover_with_inbound == null
      ? "—"
      : Number(profile.days_cover_with_inbound).toFixed(0);
  byId("cogsRead").textContent =
    economics.unit_cogs == null ? "—" : `${money(economics.unit_cogs)}/unit`;
...
function renderInventoryDecision(profile) {
  ...
  if (profile.inventory_action === "STOCKOUT") {
    ...
    read =
      "Recent velocity is not strong enough for a confident replenishment read.";
  }
...
function renderEconomicsDecision(economics) {
  if (economics.unit_cogs == null) {
    ...
    byId("econRead").textContent =
      "Add standard product cost before using product-level economics.";
    return;
  }

  byId("econDecision").textContent =
    `${money(economics.estimated_cogs_t28)} estimated COGS · 28D`;
  byId("econRead").textContent =
...
function renderVariationContext(profile, commercial, familyVariations) {
  ...
  byId("variationChips").innerHTML = Object.entries(attributes)
    .map(
      ([key, value]) =>
        `<span class="variation-chip">${escapeHtml(key)} · ${escapeHtml(value)}</span>`,
    )
    .join("");
    ...
    siblings.length > 1
      ? siblings
          ...
          .map(
            (
              item,
            ) => `<a class="sibling" href="/product?sku=${encodeURIComponent(item.sku)}">
          <div>
...
function renderAds(ads) {
  ...
  if (
    healthRead &&
    spend > 0 &&
    !healthRead.textContent.includes("Paid support is active")
  ) {
...
function render(payload) {
  ...
  byId("asof").textContent =
    `Historical through ${String(payload.business_date || "").slice(5)}`;
