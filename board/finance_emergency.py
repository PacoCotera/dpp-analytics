...
- closed_months: historical calendar months only after order-linked finance has
  substantially released, seller COGS is complete, and the following monthly
  advertising charge has posted. These are the months we aggregate for trend
  and margin analysis.
...
RELEASE_COVERAGE_MIN = float(os.getenv("FINANCE_RELEASE_COVERAGE_MIN", "0.98"))
    ...
    try:
        with connect() as conn, conn.cursor() as cur:
            ...
            if cutoff is None or last_sales_date is None:
                return {
                    "summary": {}, "current_month": {}, "closed_months": [], "finalizing_months": [],
                    "closed_aggregate": {}, "types": [], "recent": [], "cogs": [],
                    "local_time": local_time, "diagnostic": errors,
                }
            ...
            # Business sales are recognized by sale date, not Amazon posting date.
            sales_rows = _all(cur, """
            ...
            # Link released order economics back to the month of the underlying order.
            # Shipment totals already include Amazon order-level deductions. Refunds are
            # kept on the same sale month when Amazon exposes the order id.
            order_finance_rows = _all(cur, f"""
                ...
                SELECT om.month,
                       ...
                       COALESCE(sum(ft.total_amount) FILTER (
                         WHERE ft.transaction_status='RELEASED' AND ft.transaction_type='Shipment'
                       ),0)::numeric(16,2) AS shipment_net,
                       COALESCE(sum(ft.total_amount) FILTER (
                         WHERE ft.transaction_status='RELEASED' AND ft.transaction_type='Refund'
                       ),0)::numeric(16,2) AS refunds,
                       COALESCE(sum(ft.total_amount) FILTER (
                         WHERE ft.transaction_status='RELEASED' AND ft.transaction_type IN ('Shipment','Refund')
                       ),0)::numeric(16,2) AS order_finance_net
                FROM om
            ...
            # Unlinked finance postings keep their posting-month clock. Ads charged in
            # the first days of a month are treated as the settlement of the PREVIOUS
            # advertising month until Ads API performance data replaces this bridge.
            posting_rows = _all(cur, f"""
                SELECT posted_date AT TIME ZONE '{TZ}' AS local_posted,
                ...
                WHERE marketplace_id=%s AND transaction_status='RELEASED'
                  ...
                  AND transaction_type IN (
                    'ProductAdsPayment','Transfer','ServiceFee','Adjustment',
            ...
            while cursor <= current_month_start:
                ...
                finance_released = release_coverage >= RELEASE_COVERAGE_MIN
                if not historical:
                    state = "OPEN"
                ...
                # For a closed month this is a real management contribution on a
                # consistent month basis. Current month intentionally remains partial.
                contribution = None
                margin = None
                if state == "CLOSED":
            ...
            # Current month estimate deliberately stops before pretending current-month
            # ads are known. The Amazon ad bill posted this month generally closes the
            # previous advertising month; Ads API will later provide true accrual.
            current_estimate = dict(current)
            if current:
                current_estimate["estimated_contribution_before_current_ads"] = round(
                    float(current.get("amazon_order_net") or 0)
                    + float(current.get("other_amazon_postings") or 0)
                    - float(current.get("product_cogs") or 0), 2
                ) if current.get("cogs_complete") else None
                ...
                SELECT max(posted_date) AS latest_posted,
                       COALESCE(sum(total_amount) FILTER (
                         WHERE transaction_status='RELEASED' AND transaction_type='ProductAdsPayment'
                           AND posted_date>max_posted-interval '28 days'
                       ),0)::numeric(16,2) AS ads_amount_28
                FROM (
                  SELECT f.*, max(posted_date) OVER () AS max_posted
                  FROM core.financial_transaction f WHERE marketplace_id=%s
                ) x
            """, (marketplace,))
            ...
            recent = _all(cur, f"""
                ...
                FROM core.financial_transaction
                WHERE marketplace_id=%s ORDER BY posted_date DESC LIMIT 50
            """, (marketplace,))

            return {
                "summary": summary,
                "current_month": current_estimate,
                "closed_months": closed,
                "finalizing_months": finalizing,
                "closed_aggregate": aggregate,
                "monthly": months,
                "statement": current_estimate,
                "types": types,
                "daily": [],
                "breakdowns": [],
                "recent": recent,
                ...
                "close_policy": {
                    ...
                    "release_coverage_min_pct": round(RELEASE_COVERAGE_MIN * 100, 1),
                    "definition": "Closed only after order finance is substantially released, seller COGS is complete, and the following monthly Amazon advertising charge is observed.",
                },
                "diagnostic": errors,
            }
    except Exception as exc:
