...
def product_payload(connect, decorate_products, marketplace: str, sku: str) -> dict:
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute("SELECT currency,timezone,country_code FROM core.marketplace WHERE marketplace_id=%s", (marketplace,))
        market = cur.fetchone() or {}

        if cutoff and asin:
            # Reconciled customer demand is authoritative at child-ASIN grain.
            # Seller SKU remains the commercial/operational identity only.
            cur.execute(
                """
                ...
                SELECT x.*,CASE WHEN sales_prior_t28>0 THEN round(100.0*(sales_t28-sales_prior_t28)/sales_prior_t28,1) END AS delta28_pct,
            ...
            cur.execute(
                ...
                s AS (
                  ...
                  FROM ads.daily_advertised_product d JOIN ads.account a USING(account_id),c
                  WHERE a.marketplace_id=%s AND (d.advertised_sku=%s OR d.advertised_asin=%s)
                    AND d.business_date BETWEEN c.d-89 AND c.d GROUP BY d.business_date
                )
        ...
        cur.execute(
            ...
            SELECT to_char(o.created_time AT TIME ZONE mp.timezone,'MM-DD HH24:MI') AS local_time,
                   ...
                   right(o.amazon_order_id,9) AS order_short,x.units,x.customer_spend AS sales,
                   'GROSS_CUSTOMER_SPEND'::text AS sales_basis,COALESCE(o.fulfillment_status,'') AS status
    ...
    payload["metric_basis"] = {
        "currency": market.get("currency") or "MXN", "timezone": market.get("timezone"),
        "product_sales": {"id":"AMAZON_ORDERED_PRODUCT_SALES","label":"Amazon ordered-product sales","source":"ASIN Sales & Traffic / Data Kiosk","definition":"Reconciled child-ASIN sales used for 7D/28D/90D performance and portfolio economics."},
        "order_evidence": {"id":"GROSS_CUSTOMER_SPEND","label":"Shopper spend incl. IVA","source":"Amazon Orders","definition":"Item price × quantity. Settlement/proceeds amounts are excluded."},
