-- Canonical commercial offer identity for Catalog.
-- Data Kiosk traffic is CHILD-ASIN grain. A seller may have multiple seller SKUs
-- for the same ASIN; attach demand exactly once to the canonical commercial offer
-- and retain remaining SKUs as operational aliases.
...
traffic AS (
    SELECT
        ...
        c.through_date,
        COALESCE(sum(a.ordered_product_sales) FILTER (WHERE a.business_date BETWEEN c.through_date - 27 AND c.through_date),0)::numeric(14,2) AS sales_t28,
        COALESCE(sum(a.units_ordered) FILTER (WHERE a.business_date BETWEEN c.through_date - 27 AND c.through_date),0)::bigint AS units_t28,
        COALESCE(sum(a.total_order_items) FILTER (WHERE a.business_date BETWEEN c.through_date - 27 AND c.through_date),0)::bigint AS orders_t28,
        COALESCE(sum(a.sessions) FILTER (WHERE a.business_date BETWEEN c.through_date - 27 AND c.through_date),0)::bigint AS sessions_t28,
        COALESCE(sum(a.page_views) FILTER (WHERE a.business_date BETWEEN c.through_date - 27 AND c.through_date),0)::bigint AS page_views_t28,
        COALESCE(sum(a.ordered_product_sales) FILTER (WHERE a.business_date BETWEEN c.through_date - 55 AND c.through_date - 28),0)::numeric(14,2) AS sales_prior_t28,
        COALESCE(sum(a.units_ordered) FILTER (WHERE a.business_date BETWEEN c.through_date - 55 AND c.through_date - 28),0)::bigint AS units_prior_t28,
        COALESCE(sum(a.sessions) FILTER (WHERE a.business_date BETWEEN c.through_date - 55 AND c.through_date - 28),0)::bigint AS sessions_prior_t28
...
LEFT JOIN traffic t ON t.marketplace_id=r.marketplace_id AND t.asin=r.asin
...
'Canonical commercial Catalog identity. Data Kiosk demand is CHILD-ASIN grain and is attached once to one commercial seller-SKU owner per ASIN. Additional seller SKUs are aliases; structural parents are non-sellable containers. Images are exact-ASIN only.';
