-- Preserve the canonical live-parent identity model while adding the audited,
-- seller-confirmed PNC-CURRENT container. Amazon has not exposed a directional
-- child relationship for this inactive parent listing, so the exact SKU + ASIN
-- pair is guarded here as structural evidence rather than treated as an offer.
...
WITH parents AS (
    ...
    FROM mart.seller_catalog
    WHERE marketplace_id = 'A1AM78C64UM0Y8'
      AND seller_sku = 'PNC-CURRENT'
      AND asin = 'B0HGNS3FHB'
),
ranked AS (
    SELECT
...
'Canonical commercial Catalog identity. Live SKU/Catalog parent identity and guarded seller-confirmed structural evidence take precedence over persisted rolling traffic evidence; structural parents never own offer demand.';
