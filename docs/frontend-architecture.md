...
`filter`, and `sort`; transient free-text search remains session-only. Trajectory owns `window`, with `180d` as
its omitted default and `90d` and `ytd` as explicit values. Default values are omitted, invalid values normalize
to documented defaults, and unrelated parameters are preserved. Changing a choice pushes browser history;
direct load, refresh, Back, and Forward must render the same state.
