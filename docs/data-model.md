...
Backfill start dates and polling timeouts are also environment-controlled in `compose.yml`. Amazon Ads Reporting v3 waits up to 3600 seconds by default because asynchronous reports can remain queued well beyond fifteen minutes; operators can override that bound with `AMAZON_ADS_REPORT_POLL_TIMEOUT_SECONDS`. A 425 duplicate-processing response is resumable: the collector reuses Amazon's memorized report ID when supplied, or retries the identical create request until that report becomes available. Ads report generation runs in a dedicated daemon thread so an optional, slow report cannot delay the core Orders, inventory, Finance, Listings, Catalog or Data Kiosk cadence. Successful initial-history windows chain immediately until the backfill reaches yesterday; failed attempts and current reporting retain the normal configured interval.
