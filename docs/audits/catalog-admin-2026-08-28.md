...
## Temporary public-access decision

Issue [#204](https://github.com/PacoCotera/dpp-analytics/issues/204) records the 2026-08-28 decision to expose the password-protected Admin workspace through the public direct-HTTP board until SSH and an app-wide authenticated session are implemented. The application retains password authentication, rate limiting, HttpOnly/SameSite cookies, CSRF, session expiry, validation, backups and audit metadata. This does not provide transport encryption: the password and session cookie can be observed in transit. Production QA must exercise the published port, and this exception must be removed when the replacement access layer is ready.
