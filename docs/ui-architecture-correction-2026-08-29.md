...
| Finance `/finance` | What did the business earn and what is closed? | current OPEN contribution beside closed YTD, state and cutoff | management P&L table | customer-money bridge, accounting mechanics, settlement timing and COGS readiness |
| Ads `/ads` | Where does ad cost weigh most, and what deserves review first? | ad spend per $100 of seller sales plus compact API-owned readiness | ranked SKU cost-load view beside the server-owned product queue | four-week TACOS trend, conversion path and dedicated Product, Demand and Campaign views |
