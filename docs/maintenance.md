...
Brand Analytics Search Query Performance uses the existing SP-API Reports authorization, not Amazon Ads credentials. The worker requests `GET_BRAND_ANALYTICS_SEARCH_QUERY_PERFORMANCE_REPORT` for canonical current sellable ASINs, one completed calendar month per run, newest missing month first. ASIN options are space-separated and chunked under Amazon's 200-character limit. A successful first run proves the application has the Brand Analytics role for the seller; a 403 or fatal report remains an explicit `amazon_brand_analytics / search_query_performance` Data Health failure rather than being translated into “no data.”

The collector backfills up to 17 completed months and chains successful missing periods immediately in a background thread so core ingestion cadence is not blocked. Once history is complete it refreshes daily by default. `BRAND_ANALYTICS_SEARCH_QUERY_BACKFILL_MONTHS` (1–17) and `BRAND_ANALYTICS_SEARCH_QUERY_INTERVAL_SECONDS` own those bounds. Brand Analytics report creation can remain queued longer than lightweight seller reports, so `BRAND_ANALYTICS_SEARCH_QUERY_POLL_TIMEOUT_SECONDS` owns a separate one-hour default instead of inheriting the generic five-minute Reports API timeout. Each successful period replaces the requested ASIN subset transactionally, stores the raw report in `raw.api_payload`, and marks its month cursor only after canonical persistence succeeds. Data Health and manual sync use job name `search_query_performance`.
