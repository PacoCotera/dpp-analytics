...
`board/ads_rule_catalog.py` owns Advertising rule definitions during the V1-to-V2 cutover. The existing observational thresholds remain explicitly non-economic and cannot authorize a capital change. `board/ads_decisions.py` evaluates those legacy product/demand observations, stable action identifiers, action-lane allocation, product associations and demand pagination without owning another copy of the rule definitions. `ads_api.py` supplies the integrated seller-sales, traffic, conversion, Amazon-attributed performance and TACOS operands, plus a bounded Search opportunities queue joined only to canonical current offer owners. Same-month Ads search-term matches use the canonical normalized query key and are explicitly query-level, not ASIN attribution. The browser may filter and format the returned product set, but it must not invent thresholds, profitability, scaling or bid instructions. Demand is filtered, sorted and paged by the API at 20 rows per page. Changing an Ads or Search Query Performance rule requires exact-boundary server tests plus production browser verification of its rule key, version, eligibility, scenario qualification and destination.

`board/decision_contract.py` is the canonical domain-neutral DecisionCandidate version 1 owner shared by future API, server-rendering, export and Advisory consumers. It owns canonical serialization, stable IDs, fact fingerprints, required evidence metadata, lifecycle compatibility, expiry, materiality truth classes and action-class safety gates. `board/decision_store.py` is the only application persistence boundary for the `decision` schema. Candidate and experiment restatements append fingerprinted snapshots; operator dispositions, before/after changes and mature outcomes are immutable evidence. Rule lifecycle transitions are append-only and cannot move a skeleton definition into evaluation or an unapproved rule into `ACTIVE`. Contract version 1 records manual/external changes but rejects `EXECUTE`; browser code must never write these tables or reconstruct a candidate from presentation data.
...
The production probe summarizes Batch 3 evidence per SHADOW rule without exposing product identity. It reports capture and fact-state variation, distinct candidates, suppression and confidence counts, materiality distribution by declared currency, suppression codes, and any lifecycle/action-class/execution-state safety violation. `review_state` is deliberately limited to `NO_PRODUCTION_EMISSIONS`, `AWAITING_FACT_VARIATION`, or `MANUAL_REVIEW_REQUIRED`; probe output can never approve or activate a rule. Source-cutoff progression is reported independently because repeated evaluations of one unchanged fact state are not additional validation evidence.
