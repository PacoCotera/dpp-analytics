...
Brand Analytics Search Query Performance uses the existing SP-API Reports authorization, not Amazon Ads credentials. The worker requests `GET_BRAND_ANALYTICS_SEARCH_QUERY_PERFORMANCE_REPORT` for canonical current sellable ASINs at two independent grains: exact completed Sunday–Saturday weeks and completed calendar months. Missing weekly history is collected first from `BRAND_ANALYTICS_SEARCH_QUERY_WEEKLY_BACKFILL_START` (default `2025-10-01`); monthly history retains the configured 17-month maximum. One complete period is committed per run, newest missing period first, and weekly/monthly rows are never added together. ASIN options are space-separated and chunked under Amazon's 200-character limit. A successful first run proves the application has the Brand Analytics role for the seller; a 403 or fatal report remains an explicit source-specific Data Health failure rather than being translated into “no data.”

The collector backfills weekly periods to the configured business-history boundary and up to 17 completed months, chaining successful missing periods immediately in a background thread so core ingestion cadence is not blocked. Once both histories are complete it refreshes on the daily schedule. `BRAND_ANALYTICS_SEARCH_QUERY_WEEKLY_BACKFILL_START`, `BRAND_ANALYTICS_SEARCH_QUERY_BACKFILL_MONTHS` (1–17), and `BRAND_ANALYTICS_SEARCH_QUERY_INTERVAL_SECONDS` own those bounds. Brand Analytics report creation can remain queued longer than lightweight seller reports, so `BRAND_ANALYTICS_SEARCH_QUERY_POLL_TIMEOUT_SECONDS` owns a separate one-hour default instead of inheriting the generic five-minute Reports API timeout. Each successful period replaces the requested ASIN subset transactionally, stores the raw report in `raw.api_payload`, and marks that exact week or month cursor only after canonical persistence succeeds. Data Health and manual sync expose independent `search_query_performance_weekly` and `search_query_performance` job names.
