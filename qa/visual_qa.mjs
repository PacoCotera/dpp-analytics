...
async function verifyAds(page, view = "overview") {
  const status = await page.evaluate(
    async () =>
      (await (await fetch("/api/ads", { cache: "no-store" })).json()).status,
  );
    ...
    return {
      ...
      singleRead: Boolean(
        brief &&
          ...
          !document.querySelector(".state-read"),
      ),
      hierarchy: Boolean(
        top(brief) < top(rhythm) &&
          top(rhythm) < top(attention) &&
          top(attention) < top(health),
      ),
...
async function verifyProductWorkspace(page) {
  ...
  if (
    !payload.ads?.through_date ||
    !Number(payload.ads?.observed_ads_days || 0)
  ) {
    const adsState = (
    ...
    if (
      adsState !== "Ads access pending" ||
      adsDecision !== "Ads integration ready"
    )
      throw new Error(
        `Product Ads pending language mismatch: ${adsState} / ${adsDecision}`,
      );
  }
...
async function verifyTrajectory(page) {
  ...
  if (!state.mobile && (!state.guideOpen || !state.referenceOpen))
    throw new Error(
      `Trajectory desktop evidence is collapsed: ${JSON.stringify(state)}`,
    );
}
