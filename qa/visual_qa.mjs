...
async function verifyProductWorkspace(page) {
  ...
  if (orderEvidence.statuses.some((status) => status.trim() === "PENDING"))
    throw new Error("Product order evidence exposed raw PENDING status");
}
...
async function verifySalesOrders(page) {
  ...
  const state = await page.evaluate(async () => {
    ...
    return {
      ...
      statuses: rows.map(
        (row) => row.querySelector(".order-status-pill")?.textContent || "",
      ),
    };
  });
  ...
  if (state.statuses.some((status) => status.trim() === "PENDING"))
    throw new Error("Sales Orders exposed raw PENDING status");
  if (
    state.mobile &&
    state.total > 10 &&
    (state.visible !== 10 ||
...
const scenarios = [
  [
    ...
    ["mobile", "desktop"],
    async (p) => {
      await wait(p, "#rhythm .dpp-bar");
      await wait(p, "#dayPicker .day-choice");
    },
  ],
  ...
  [
    ...
    ["mobile", "desktop"],
    async (p) => {
      await p.locator('button[data-view="products"]').click();
      await wait(p, "#skuRows tr");
    },
  ],
