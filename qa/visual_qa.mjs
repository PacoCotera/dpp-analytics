...
const wait = (page, selector) =>
  page.locator(selector).first().waitFor({ state: "visible", timeout: 5000 });

async function assertWorkspaceLandmarks(page, names) {
  const missing = await page.evaluate((expected) => {
    const available = new Set(
      [...document.querySelectorAll("[data-dpp-qa]")].map((element) =>
        element.getAttribute("data-dpp-qa"),
      ),
    );
    return expected.filter((name) => !available.has(name));
  }, names);
  if (missing.length)
    throw new Error(`Missing workspace landmarks: ${missing.join(", ")}`);
}
...
async function verifyControlTrustAppearance(page, expectedProfile) {
  ...
  await page.waitForFunction(
    () => {
      if (document.body.classList.contains("finance-page"))
        return Boolean(document.querySelector("#currentLines .finance-line"));
      if (document.body.classList.contains("ads-page"))
        return (
          !document.getElementById("readyState")?.hidden ||
          !document.getElementById("emptyState")?.hidden
        );
      if (document.body.classList.contains("data-health-page"))
        return (
          document.getElementById("summaryCount")?.textContent?.trim() !== "—"
        );
      if (document.body.classList.contains("admin-shell"))
        return Boolean(document.getElementById("loginPanel"));
      return false;
    },
    null,
    { timeout: 10000 },
  ...
  const state = await page.evaluate((profileId) => {
    ...
    return {
      ...
      semanticTokens: [
        ...
        "--dpp-healthy-surface",
        "--dpp-warning-surface",
        "--dpp-critical-surface",
      ].every((token) => css.getPropertyValue(token).trim()),
      darkSurfaces:
        !["midnight-dark", "aubergine-dark", "weyland"].includes(profileId) ||
        surfaces.every(
          (node) => channelMax(getComputedStyle(node).backgroundColor) < 180,
        ),
      controlFloor: controls.every(
        (node) => node.getBoundingClientRect().height >= 40,
      ),
      metadataFloor:
        Number.parseFloat(css.getPropertyValue("--dpp-metadata-size")) >= 14,
      weylandType:
        profileId !== "weyland" ||
  ...
  if (
    ...
    throw new Error(
      `Control/trust ${expectedProfile} appearance mismatch: ${JSON.stringify(state)}`,
    );
  }
}
...
async function verifyAdmin(page) {
  ...
  const state = await page.evaluate(() => {
    ...
    const evidence = [
      ...document.querySelectorAll(
        ".admin-shell .kicker,.admin-shell .page-header__description,.admin-eyebrow,.admin-login p,.admin-status",
      ),
    ].filter(visible);
    const controls = [
      ...document.querySelectorAll("#loginPanel input,#loginPanel button"),
    ].filter(visible);
    return {
      evidenceFloor: evidence.every(
  ...
  if (!state.evidenceFloor || !state.controlFloor) {
    throw new Error(
      `Admin login presentation mismatch: ${JSON.stringify(state)}`,
    );
  }
}
...
async function verifyAds(page, view = "overview") {
  await assertWorkspaceLandmarks(page, [
    "ads-workspace-header",
    "ads-operating-evidence",
  ]);
  ...
  if (
    navigation.mobile &&
    (navigation.pageOverflow > 1 ||
      navigation.tabCount !== 5 ||
      !navigation.containedOverflow)
  ) {
    throw new Error(
      `Ads mobile navigation mismatch: ${JSON.stringify(navigation)}`,
    );
  }
  const payload = await page.evaluate(
    async () => await (await fetch("/api/ads", { cache: "no-store" })).json(),
  );
  if (payload.connection?.state !== "READY" || payload.status !== "ready") {
    ...
    const disconnected = await page.evaluate((connectionDetail) => {
      ...
      return {
        overviewEnabled:
          tabs[0]?.dataset.adsView === "overview" && !tabs[0].disabled,
        drillsDisabled: tabs
          .slice(1)
          .every(
            (tab) =>
        ...
        explained: Boolean(
          note &&
            !note.hidden &&
            note.textContent.includes(connectionDetail || ""),
        ),
        contained:
          document.documentElement.scrollWidth <=
          document.documentElement.clientWidth + 2,
    ...
    if (
      !disconnected.overviewEnabled ||
      !disconnected.drillsDisabled ||
      !disconnected.explained ||
      !disconnected.contained
    ) {
      throw new Error(
        `Ads disconnected-state presentation mismatch: ${JSON.stringify(disconnected)}`,
      );
    }
    return;
  }
  ...
  const state = await page.evaluate((apiActions) => {
    ...
    const controls = [
      ...document.querySelectorAll(".ads-page .subnav__item,.ads-action-open"),
    ].filter(visible);
    const reasons = [
      ...document.querySelectorAll("#actionQueue .ads-action-body p"),
    ...
    return {
      ...
      tabsContained:
        Boolean(
          tabsRect &&
            tabsRect.left >= -2 &&
            tabsRect.right <= window.innerWidth + 2,
        ) &&
        document.documentElement.scrollWidth <=
          document.documentElement.clientWidth + 2,
      evidenceFloor: evidence.every(
      ...
      apiReasons:
        ...
        JSON.stringify(apiActions.map((action) => String(action.reason || ""))),
      browserActionCells: document.querySelectorAll(
        "#targetRows .ads-action,#searchTermRows .ads-action",
      ).length,
    };
  }, payload.actions || []);
  if (
    ...
    !state.controlFloor ||
    !state.apiReasons ||
    state.browserActionCells
  ) {
    throw new Error(
      `Ads evidence/control presentation mismatch: ${JSON.stringify(state)}`,
    );
  }
}
...
async function verifySalesOverview(page) {
  ...
  const state = await page.evaluate(() => {
    ...
    const ruleTrigger = [
      ...document.querySelectorAll(".sales-page .rule-trigger"),
    ...
    return {
      ...
      primaryVisible: Boolean(
        primary && primary.getBoundingClientRect().height > 0,
      ),
      ...
      ruleTriggerFont: Number.parseFloat(
        ruleTrigger ? getComputedStyle(ruleTrigger).fontSize : "0",
      ),
      todayLinkHeight: todayLink?.getBoundingClientRect().height || 0,
      todayLinkFont: Number.parseFloat(
        todayLink ? getComputedStyle(todayLink).fontSize : "0",
      ),
    };
  });
  if (
    ...
    throw new Error(
      `Sales utility control floor mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (
    state.mobile &&
    (!state.signalsBeforeChart ||
      state.referenceOpen ||
      !state.primaryVisible ||
      !state.todayVisible)
  ) {
    throw new Error(
      `Sales Overview mobile hierarchy mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (!state.mobile && state.referenceOpen)
    throw new Error(
      "Sales Overview desktop reference context should remain secondary",
    );
}
...
async function verifySalesProducts(page) {
  ...
  const state = await page.evaluate(async () => {
    const payload = await (
      await fetch("/api/sales", { cache: "no-store" })
    ).json();
    ...
    return {
      ...
      visible: rows.filter((row) => getComputedStyle(row).display !== "none")
        .length,
      ...
      driverTab: document
        .querySelector('button[data-view="products"]')
        ?.textContent?.trim(),
      ...
      readPopulated: [
        ...document.querySelectorAll(".product-read__value"),
      ].every(
        (item) => item.textContent.trim() && item.textContent.trim() !== "—",
      ),
      ...
      thumbnailsMatch: rows.every((row, index) =>
        expected[index]?.image_url
          ? Boolean(row.querySelector(".product-thumb"))
          : true,
      ),
    };
  });
  if (
    ...
    throw new Error(
      `Sales Products contract mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (
    state.mobile &&
    ...
    throw new Error(
      `Sales Products mobile density mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (!state.mobile && state.visible !== state.total)
    throw new Error(
      `Sales Products desktop rows hidden: ${JSON.stringify(state)}`,
    );
}
...
async function verifyToday(page) {
  ...
  const state = await page.evaluate(() => {
    ...
    const priority = [
      ...document.querySelectorAll(".today-products-priority .today-product"),
    ];
    const rhythmKpis = [...document.querySelectorAll(".rhythm-kpi")];
    const tops = rhythmKpis.map((item) =>
    ...
    return {
      ...
      recipeMatch: Boolean(
        main &&
          [...main.children]
            .filter((item) => item.matches("section"))
            .every(
              (section, index) =>
                section ===
                [overview, rhythm, operations, evidenceSection][index],
            ) &&
          [...main.children].filter((item) => item.matches("section"))
            .length === 4,
      ),
  ...
  if (
    ...
    throw new Error(
      `Today mobile hierarchy mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (
    !state.mobile &&
    (!state.evidenceOpen || (state.referencePresent && !state.referenceOpen))
  )
    throw new Error(
      `Today desktop evidence is collapsed: ${JSON.stringify(state)}`,
    );
}
...
async function verifyBusiness(page) {
  ...
  const state = await page.evaluate(async () => {
    ...
    const exceptions = (payload.inventory || []).filter((item) =>
      ["STOCKOUT", "PRODUCE", "PLAN"].includes(
        String(item.action || "").toUpperCase(),
      ),
    );
    const total = Math.max(
      exceptions.length,
      Number(payload.inventory_summary?.needs_action || 0),
    );
    const main = document.querySelector("main.home-main");
    const sections = main
      ? [...main.children].filter((element) => element.matches("section"))
      : [];
    const overview = document.querySelector(
      '[data-dpp-qa="business-overview"]',
    );
    const demand = document.querySelector('[data-dpp-qa="business-demand"]');
    const decisions = document.querySelector(
      '[data-dpp-qa="business-decisions"]',
    );
    const health = document.querySelector('[data-dpp-qa="business-health"]');
    const dataHealthCard = document.querySelector(
      '.business-health-card[href="/data-health"]',
    );
    ...
    const explanation = String(
      payload.business_momentum?.explanation || "",
    ).trim();
    ...
    return {
      evidenceHeadline: document
        .getElementById("stateHeadline")
        ?.textContent?.trim(),
      expectedHeadline,
      activeNav: document
        .querySelector(".nav-primary-set a.active")
        ?.textContent?.trim(),
      ...
      severityBadges: document.querySelectorAll(
        ".attention-item .severity-badge",
      ).length,
      ...
      expectedMore:
        exceptions.length > 0 && total > Math.min(4, exceptions.length),
      healthDomains: document.querySelectorAll(".business-health-card").length,
      healthContractId: healthContract.contract_id,
      healthValue: dataHealthCard
        ?.querySelector(".business-health-card__value")
        ?.textContent?.trim(),
      ...
      rhythmCurrentBand: document.querySelectorAll(
        "#spark .home-rhythm__current-week",
      ).length,
      ...
      rhythmWeekendDays: document.querySelectorAll(
        "#spark .home-rhythm__bar--weekend",
      ).length,
      signalCopy: document
        .querySelector(
          '[data-dpp-qa="business-demand"] .section-header__description',
        )
        ?.textContent?.replace(/\s+/g, " ")
        .toLowerCase()
        .includes("seven-day signal"),
      productDriversRemoved: !document.querySelector(
        ".drivers, #movers, .driver",
      ),
      adsVisible: Boolean(
        ads && !ads.hidden && getComputedStyle(ads).display !== "none",
      ),
  ...
  if (
    ...
    (state.healthConditionCount > 0 &&
      !state.healthAffectedDomains.every((domain) =>
        state.healthCopy.includes(domain),
      )) ||
    ...
    throw new Error(
      `Business decision-board contract mismatch: ${JSON.stringify(state)}`,
    );
  }
}
async function verifyDataHealth(page) {
    ...
    return {
      ...
      contractComplete: jobs.every(
        (job) =>
          ...
          Number(job.stale_after_seconds) >=
            Number(job.expected_interval_seconds) &&
      ...
      incidentStructure: incidents.every(
        (incident) =>
      ...
      compactCoverage: Boolean(
        document.querySelector(".domain-summary .domain-chip"),
      ),
      healthContractId: healthContract.contract_id,
      sharedContract:
        JSON.stringify(healthContract) ===
        JSON.stringify(home.health_contract || {}),
      scopeDefined:
      ...
      stateMappingDefined:
        healthContract.domains?.filter((domain) => domain.critical).length ===
          5 &&
        Number.isInteger(healthContract.overall?.active_condition_count) &&
        Array.isArray(healthContract.overall?.affected_domains),
      renderedConditionCount: Number(
        document.getElementById("summaryCount")?.textContent,
      ),
      expectedConditionCount: Number(
        healthContract.overall?.active_condition_count || 0,
      ),
      warehouseClosed: !document
        .querySelector(".warehouse-reference")
        ?.hasAttribute("open"),
      warehouseSummaryHeight:
        document
          .querySelector(".warehouse-reference summary")
      ...
      mobilePipelineMetrics:
        ...
        ([...document.querySelectorAll("#jobs .health-job")].length ===
          jobs.length &&
          [...document.querySelectorAll("#jobs .health-job")].every((row) =>
            [
              ...
              (metric) =>
                metric && window.getComputedStyle(metric).display !== "none",
            ),
          )),
      pipelineTable:
        document
          .querySelector('.pipeline-panel [role="table"]')
          ?.getAttribute("aria-label") === "Pipeline health" &&
        document.querySelectorAll('.health-job--header [role="columnheader"]')
          .length === 5 &&
        [...document.querySelectorAll("#jobs .health-job")].every(
          (row) =>
            row.getAttribute("role") === "row" &&
            row.querySelectorAll('[role="cell"]').length === 5,
        ),
      mobileHeaderAvailable:
        window.innerWidth > 900 ||
        window.getComputedStyle(document.querySelector(".health-job--header"))
          .display !== "none",
      evidenceFloor: [
        ...document.querySelectorAll(
      ...
      controlFloor: [
        ...document.querySelectorAll(
          ".data-health-page button,.warehouse-reference summary",
        ),
      ]
        .filter((node) => node.getClientRects().length > 0)
  ...
  if (
    ...
    throw new Error(
      `Data Health diagnostic contract mismatch: ${JSON.stringify(state)}`,
    );
  }
}
...
async function catalogSemantic(page) {
  return page.evaluate(async () => {
    ...
    if (!data.summary?.taxonomy_override_configured)
      errors.push("seller taxonomy is not configured");
    const unmappedSkus = data.summary?.taxonomy_unmapped_skus || [];
    if (unmappedSkus.length)
      errors.push(
        `sellable SKUs missing seller taxonomy: ${unmappedSkus.join(", ")}`,
      );
    for (const family of data.families || []) {
      // Amazon lifecycle markers are operational metadata, never commercial family names.
      if (/\b(actual|archivo)\b/i.test(String(family.name || "")))
        errors.push(
          `${family.family_asin}: raw Amazon lifecycle label leaked into family name`,
        );
      const members = (family.members || []).filter((x) =>
        ["SELLABLE_VARIATION", "SELLABLE_STANDALONE"].includes(x.product_role),
      );
      if (!members.length) continue;
      const sum = (key) => members.reduce((n, x) => n + Number(x[key] || 0), 0);
      const sales = sum("sales_t28"),
        units = sum("units_t28"),
        sessions = sum("sessions_t28");
      if (!close(family.sales_t28, sales))
        errors.push(`${family.family_asin}: family sales != child rollup`);
      if (Number(family.units_t28 || 0) !== units)
        errors.push(`${family.family_asin}: family units != child rollup`);
      if (Number(family.sessions_t28 || 0) !== sessions)
        errors.push(`${family.family_asin}: family sessions != child rollup`);
      if (
        Number(family.available || 0) !== sum("available") ||
        Number(family.inbound || 0) !== sum("inbound")
      )
        errors.push(`${family.family_asin}: family inventory != child rollup`);
      const expectedCvr =
        sessions > 0 ? Math.round((10000 * units) / sessions) / 100 : null;
      if (
        expectedCvr === null
          ? family.conversion_t28_pct != null
          : !close(family.conversion_t28_pct, expectedCvr)
      )
        errors.push(`${family.family_asin}: family CVR not recomputed`);
      if (family.parent && family.primary_state === "STRUCTURAL_PARENT")
        errors.push(
          `${family.family_asin}: structural parent used as diagnosis`,
        );
      if (
        (family.members || []).some(
          (x) => x.product_role === "STRUCTURAL_PARENT",
        )
      )
        errors.push(
          `${family.family_asin}: structural parent leaked into members`,
        );
    }
    const dimensionNames = Object.keys(data.dimensions || {});
    if (
      (data.summary?.amazon_dimension_coverage || 0) > 0 &&
      !dimensionNames.length
    )
      errors.push(
        "variation metadata exists but dimensional rollups are empty",
      );
    for (const [dimension, rows] of Object.entries(data.dimensions || {}))
      for (const row of rows || []) {
        const sessions = Number(row.sessions_t28 || 0),
          units = Number(row.units_t28 || 0);
        const expected =
          sessions > 0 ? Math.round((10000 * units) / sessions) / 100 : null;
        if (
          expected === null
            ? row.conversion_t28_pct != null
            : !close(row.conversion_t28_pct, expected)
        )
          errors.push(
            `${dimension}/${row.value}: dimensional CVR not recomputed`,
          );
      }
    return {
      errors,
      dimensionNames,
      pairCount: (data.dimension_pairs || []).length,
    };
  });
}
...
async function verifyCatalog(page) {
  ...
  if (semantic.errors?.length)
    throw new Error(`Catalog semantic QA: ${semantic.errors.join("; ")}`);
  const openCount = await page.locator(".family[open]").count();
  if (openCount)
    throw new Error(
      `Catalog default comparison view has ${openCount} family expansions open`,
    );
      ...
      const metrics = summary
        ? [
            ...summary.querySelectorAll(
              ":scope > .metric-sales, :scope > .metric-funnel, :scope > .metric-stock",
            ),
          ]
        : [];
      ...
      return {
        duplicateTitleVisible: Boolean(
          document.querySelector(".catalog-title") &&
            getComputedStyle(document.querySelector(".catalog-title"))
              .display !== "none",
        ),
        economicsVisible: Boolean(
          economics && getComputedStyle(economics).display !== "none",
        ),
        metricCount: metrics.length,
        metricTopSpread: tops.length
          ? Math.max(...tops) - Math.min(...tops)
          : null,
      };
    });
    if (mobile.duplicateTitleVisible)
      throw new Error("Catalog mobile repeats the portfolio title");
    if (mobile.economicsVisible)
      throw new Error(
        "Catalog mobile exposes desktop economics in the family summary",
      );
...
async function verifyCatalogMode(page, mode) {
  ...
  const semantic = await catalogSemantic(page);
  if (
    mode.startsWith("dimension:") &&
    !semantic.dimensionNames?.includes(mode.split(":")[1])
  )
    throw new Error(`Catalog dimension ${mode.split(":")[1]} unavailable`);
  if (mode === "pair" && !semantic.pairCount)
    throw new Error("Catalog variation-combination rollups unavailable");
  ...
  if (mobileDensity.mobile && mobileDensity.total > 6) {
    if (
      !mobileDensity.disclosure ||
      mobileDensity.disclosureOpen ||
      mobileDensity.visible !== 6
    ) {
      throw new Error(
        `Catalog mobile density mismatch: ${JSON.stringify(mobileDensity)}`,
      );
    }
  }
}
...
async function verifyProductWorkspace(page) {
  ...
  const payload = await page.evaluate(
    async () =>
      await (
        await fetch("/api/product?sku=PNC-001", { cache: "no-store" })
      ).json(),
  );
  const expected = payload.profile?.product;
  if (payload.profile?.label_source !== "mapping" || !expected)
    throw new Error(
      `Product mapped-name contract mismatch: ${payload.profile?.product || "blank"} / ${payload.profile?.label_source || "unknown source"}`,
    );
  const renderedName = (
  ...
  if (renderedName !== expected)
    throw new Error(
      `Product hero canonical name mismatch: ${renderedName || "blank"}`,
    );
  ...
  if (payload.commercial?.catalog_membership !== "DELETED") {
    const adsState = (
      (await page.locator("#adsState").textContent()) || ""
    ).trim();
    const adsDecision = (
      (await page.locator("#adsDecision").textContent()) || ""
    ).trim();
    const connection = payload.ads?.connection || {};
    if (adsState !== connection.badge || adsDecision !== connection.headline)
      throw new Error(
        `Product Ads state-machine mismatch: ${adsState} / ${adsDecision}`,
      );
  }
  ...
  const mobileHierarchy = await page.evaluate(() => {
    ...
    return {
      ...
      factsVisible: Boolean(
        facts && window.getComputedStyle(facts).display !== "none",
      ),
      decisionsAfterChart: Boolean(
        decisions &&
          chart &&
          decisions.getBoundingClientRect().top >=
            chart.getBoundingClientRect().top,
      ),
      referenceSummaryHeight: summary?.getBoundingClientRect().height || 0,
      catalogTitleVisible: Boolean(
        document.querySelector(".hero-catalog-title")?.getBoundingClientRect()
          .height,
      ),
  ...
  if (
    ...
    throw new Error(
      `Product mobile hierarchy mismatch: ${JSON.stringify(mobileHierarchy)}`,
    );
  }
  if (!mobileHierarchy.mobile && mobileHierarchy.referenceOpen)
    throw new Error(
      "Product desktop secondary context should remain subordinate",
    );

  await page.locator('[data-metric="units"]').click();
  const unitsChartLabel = await page
    .locator("#chart")
    .getAttribute("aria-label");
  if (!String(unitsChartLabel || "").includes("daily units"))
    throw new Error(
      `Product units chart toggle failed: ${unitsChartLabel || "missing label"}`,
    );
  ...
  const orderEvidence = await page.evaluate(() => {
    ...
    return {
      ...
      visible: orders.filter(
        (order) => order.getBoundingClientRect().height > 0,
      ).length,
      statuses: orders.map(
        (order) => order.querySelector(".order-status-pill")?.textContent || "",
      ),
    };
  });
  if (!orderEvidence.count || !orderEvidence.structured)
    throw new Error(
      `Product order evidence structure mismatch: ${JSON.stringify(orderEvidence)}`,
    );
  if (orderEvidence.count > 6 && orderEvidence.visible !== 6)
    throw new Error(
      `Product order evidence preview mismatch: ${JSON.stringify(orderEvidence)}`,
    );
  ...
  if (leakedOrderPending.length)
    throw new Error(
      `Product order evidence leaked raw pending language: ${JSON.stringify(leakedOrderPending)}`,
    );
  if (mobileHierarchy.mobile)
    await page.locator("#ordersPanel > summary").click();
}
...
async function verifyProductZeroDemand(page) {
  ...
  const salesState = await page.evaluate(async () => {
    const payload = await (
      await fetch("/api/product?sku=PNC-001L", { cache: "no-store" })
    ).json();
    const rows = (payload.series || []).slice(-28);
    return {
      seriesAllZero:
        rows.length > 0 && rows.every((row) => Number(row.sales || 0) === 0),
      message: document.querySelector("#chart .dpp-muted")?.textContent?.trim(),
      ticks: [...document.querySelectorAll("#chart .dpp-axis text")].map(
  ...
  if (
    ...
    throw new Error(
      `Product zero-sales chart mismatch: ${JSON.stringify(salesState)}`,
    );
  }
  ...
  const unitsState = await page.evaluate(async () => {
    ...
    return {
      seriesAllZero:
        rows.length > 0 && rows.every((row) => Number(row.units || 0) === 0),
      message: document.querySelector("#chart .dpp-muted")?.textContent?.trim(),
      ticks: [...document.querySelectorAll("#chart .dpp-axis text")].map(
        (node) => node.textContent.trim(),
      ),
  ...
  if (
    ...
    throw new Error(
      `Product zero-units chart mismatch: ${JSON.stringify(unitsState)}`,
    );
  }
}
...
async function verifyTrajectory(page) {
  ...
  const state = await page.evaluate(() => {
    ...
    const priority = [
      ...document.querySelectorAll(".structure-priority .structure-card"),
    ];
    const evidence = [
      ...document.querySelectorAll(
    ...
    const ruleTrigger = [
      ...document.querySelectorAll(".trajectory-page .rule-trigger"),
    ].find((element) => element.getClientRects().length > 0);
    return {
      mobile,
      emptyPaidVisible: Boolean(
        paidEmpty && paid && window.getComputedStyle(paid).display !== "none",
      ),
      ...
      chartContained: Boolean(
        chartScroll && chartScroll.scrollWidth > chartScroll.clientWidth,
      ),
      evidenceFloor: evidence.every(
        (element) =>
  ...
  if (
    ...
    throw new Error(
      `Trajectory evidence/control floor mismatch: ${JSON.stringify(state)}`,
    );
  if (
    state.mobile &&
    ...
    throw new Error(
      `Trajectory mobile hierarchy mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (!state.mobile && (state.guideOpen || state.referenceOpen))
    throw new Error(
      `Trajectory secondary evidence is expanded: ${JSON.stringify(state)}`,
    );
}
async function verifyInventory(page) {
  ...
  const contract = await page.evaluate(async () => {
    ...
    const holdCount = (payload.rows || []).filter(
      (row) => row.action === "HOLD",
    ).length;
    ...
    return {
      ...
      visibleReferenceCards: [
        ...document.querySelectorAll(".inv-card--reference"),
  ...
  if (contract.holdCount && !contract.hasDetails)
    throw new Error(
      "Inventory mobile default is missing collapsed reference inventory",
    );
  if (contract.detailsOpen || contract.visibleReferenceCards)
    throw new Error(
      "Inventory mobile default exposes the no-velocity reference wall",
    );
}
...
async function assertFinanceChartMarks(page, label) {
  ...
  const marks = await page
    .locator("#progression rect, #progression path, #progression line")
    .count();
...
async function chooseClosedFinanceMonth(page) {
  ...
  await page
    .locator('button[data-finance-window="month"][aria-selected="true"]')
    .waitFor({ state: "visible", timeout: 5000 });
  await wait(page, "#monthPicker");
  const options = await page
    .locator("#monthPicker option")
    .evaluateAll((items) =>
      items.map((item) => ({
        value: item.value,
  ...
  if (!options.length)
    throw new Error("Finance Month view has no accounting-month options");
  const closedOption = options.find((option) => !option.text.includes("OPEN"));
  if (!closedOption)
    throw new Error(
      "Finance Month view has no closed month available for immutable drill-down QA",
    );
  await page.locator("#monthPicker").selectOption(closedOption.value);
  await assertFinanceChartMarks(page, "closed month");
  const closedState = (
    (await page.locator("#progressionState").textContent()) || ""
  ).trim();
  if (!closedState || closedState.includes("OPEN"))
    throw new Error(
      `Finance closed-month drill-down has invalid state: ${closedState || "blank"}`,
    );
}
...
async function verifyFinanceWindows(page) {
  ...
  for (const windowKey of buttons) {
    ...
    await page
      .locator(
        `button[data-finance-window="${windowKey}"][aria-selected="true"]`,
      )
      .waitFor({ state: "visible", timeout: 5000 });
  ...
  await page
    .locator('button[data-finance-window="ytd"][aria-selected="true"]')
    .waitFor({ state: "visible", timeout: 5000 });
...
async function verifyFinanceReport(page) {
  ...
  if (isMobile) {
    if (await page.locator(".finance-read--current-summary").isVisible()) {
      throw new Error(
        "Finance mobile repeats the current-month contribution summary",
      );
    }
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      mobileHeaderAvailable:
        !mobile ||
        (tableHead && getComputedStyle(tableHead).display !== "none"),
      tableRelationships:
        document.querySelectorAll('#history thead th[scope="col"]').length ===
          8 &&
        document.querySelectorAll("#history tbody tr").length ===
          document.querySelectorAll('#history tbody th[scope="row"]').length,
      anchoredSections:
        document.querySelectorAll(".finance-page h2").length >= 7,
    };
  });
  if (
    !state.evidenceFloor ||
    ...
    !state.anchoredSections
  ) {
    throw new Error(
      `Finance hierarchy/accessibility presentation mismatch: ${JSON.stringify(state)}`,
    );
  }
}
...
async function verifySalesOrders(page) {
  ...
  const state = await page.evaluate(async () => {
    ...
    return {
      ...
      evidenceOpen: Boolean(
        document.getElementById("orderEvidence")?.hasAttribute("open"),
      ),
      ...
      orderLabels: rows.every(
        (row) =>
          row.querySelector(".order-moment__label")?.textContent?.trim() ===
            "Order" &&
          row
            .querySelector(".order-status-cell__label")
            ?.textContent?.trim() === "Fulfillment",
      ),
      structured: rows.every(
        (row) =>
      ...
      namedItems: [
        ...document.querySelectorAll(".sales-order-item strong"),
      ...
      itemContracts: rows.map((row, index) => {
        ...
        return {
          ...
          fallbackVisible: Boolean(
            row.querySelector(".sales-order-items__empty"),
          ),
        };
      }),
      statuses: rows.map(
        (row) =>
          row.querySelector(".order-status-pill")?.textContent?.trim() || "",
      ),
    };
  });
  if (
    ...
    throw new Error(
      `Sales Orders structure mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (
    state.mobile &&
    ...
    throw new Error(
      `Sales Orders mobile boundaries mismatch: ${JSON.stringify(state)}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Sales Orders mobile density mismatch: ${JSON.stringify(state)}`,
    );
  }
  if (!state.mobile && state.visible !== state.total) {
    throw new Error(
      `Sales Orders desktop rows hidden: ${JSON.stringify(state)}`,
    );
  }
  const leakedPending = state.statuses.filter((status) =>
    /\bpending(?:_availability)?\b/i.test(status),
  );
  if (leakedPending.length) {
    throw new Error(
      `Sales Orders leaked raw pending language: ${JSON.stringify(leakedPending)}`,
    );
  }
}
...
const scenarios = [
  ...
  [
    ...
    "/catalog",
    ["mobile", "desktop"],
    (p) => verifyCatalogMode(p, "dimension:design"),
  ],
  [
    "catalog-ruling",
    "/catalog",
    ["mobile", "desktop"],
    (p) => verifyCatalogMode(p, "dimension:ruling"),
  ],
  [
    "catalog-combinations",
    "/catalog",
    ["mobile", "desktop"],
    (p) => verifyCatalogMode(p, "pair"),
  ],
  [
    "catalog-sku",
    "/catalog",
    ["mobile", "desktop"],
    (p) => verifyCatalogMode(p, "sku"),
  ],
  [
    "product-pnc-001",
    "/product?sku=PNC-001",
  ...
  [
    "product-pnc-001l-zero",
    "/product?sku=PNC-001L",
    ["desktop"],
    verifyProductZeroDemand,
  ],
...
for (const scenario of scenarios)
  for (const viewportName of scenario.views) {
    ...
    const result = {
      ...
      profile: scenario.profile || "warm-studio",
    ...
    try {
      ...
      if (!response?.ok())
        throw new Error(
          `navigation returned ${response?.status() || "no response"}`,
        );
      await page
      ...
      result.metrics = await page.evaluate(
        ({ viewportName }) => {
          ...
          const rawUncontained = mainElements.filter((element) => {
            ...
            return (
              (bounds.left < -2 || bounds.right > window.innerWidth + 2) &&
              !hasHorizontalScrollAncestor(element)
            );
          });
          const uncontainedElements = rawUncontained
            .filter(
              (element) =>
                !rawUncontained.some(
          ...
          const clippedContainers = mainElements
            .filter((element) => {
            ...
            .map((element) => ({
              ...
              deltaY: element.scrollHeight - element.clientHeight,
            }));
          ...
          return {
            ...
            activeTab:
              document
                .querySelector(
                  ".tabs button.active,.view-tabs button.active,.analysis-modes button.active",
                )
                ?.textContent?.trim() || null,
            scrollWidth,
            scrollHeight: Math.max(doc.scrollHeight, body.scrollHeight),
            horizontalOverflowPx: Math.max(0, scrollWidth - doc.clientWidth),
            uncontainedElements,
            clippedContainers,
      ...
      if (
        viewportName === "mobile" &&
        result.metrics.uncontainedElements.length
      ) {
        errors.push(
          `mobile component overflow: ${JSON.stringify(result.metrics.uncontainedElements)}`,
        );
      }
      if (
        viewportName === "mobile" &&
        result.metrics.clippedContainers.length
      ) {
        errors.push(
          `mobile clipped content: ${JSON.stringify(result.metrics.clippedContainers)}`,
        );
      }
      ...
      await page
        .screenshot({ path: path.join(outDir, fileName), fullPage: true })
        .catch(() => {});
      result.screenshot = fileName;
    }
    results.push(result);
...
const summary = {
  ...
  failedResponseCount: results.reduce(
    (n, x) => n + x.failedResponses.length,
    0,
  ),
  horizontalOverflowCaptures: results.filter(
    (x) => (x.metrics?.horizontalOverflowPx || 0) > 2,
  ).length,
  uncontainedComponentCaptures: results.filter(
    (x) => (x.metrics?.uncontainedElements?.length || 0) > 0,
  ).length,
  clippedContentCaptures: results.filter(
    (x) => (x.metrics?.clippedContainers?.length || 0) > 0,
  ).length,
  smallTextSignals: results.reduce(
    (n, x) => n + (x.metrics?.smallTextCount || 0),
    0,
  ),
  smallTapTargetSignals: results.reduce(
    (n, x) => n + (x.metrics?.smallTapTargetCount || 0),
    0,
  ),
  results,
};
...
for (const r of results)
  lines.push(
    `| ${r.scenario} | ${r.viewport} ${r.width}×${r.height} | ${r.metrics?.activeTab ?? "—"} | ${r.metrics?.horizontalOverflowPx ?? "—"}px | ${r.metrics?.smallTextCount ?? "—"} | ${r.metrics?.smallTapTargetCount ?? "—"} | ${r.errors.length} |`,
  );
lines.push(
  "",
  "## Signals",
  "",
  `- Horizontal overflow: ${summary.horizontalOverflowCaptures} capture(s)`,
  `- Uncontained mobile components: ${summary.uncontainedComponentCaptures} capture(s)`,
  `- Clipped mobile content: ${summary.clippedContentCaptures} capture(s)`,
  `- Small-text signals: ${summary.smallTextSignals}`,
  `- Small tap-target signals: ${summary.smallTapTargetSignals}`,
  `- Failed local HTTP responses: ${summary.failedResponseCount}`,
  `- Browser/page errors: ${summary.consoleErrorCount}`,
  "",
  "_Screenshots remain the source of truth for visual judgment._",
  "",
);
