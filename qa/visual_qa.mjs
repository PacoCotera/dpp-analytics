...
async function verifyAds(page, view = 'overview') {
  const status = await page.evaluate(
    async () => (await (await fetch('/api/ads', { cache: 'no-store' })).json()).status,
  );
  ...
  return page.evaluate(async () => {
    ...
    for (const family of data.families || []) {
      ...
      if (/\b(actual|archivo)\b/i.test(String(family.name || '')))
        errors.push(`${family.family_asin}: raw Amazon lifecycle label leaked into family name`);
      ...
      if (family.parent && family.primary_state === 'STRUCTURAL_PARENT')
        errors.push(`${family.family_asin}: structural parent used as diagnosis`);
      if ((family.members || []).some((x) => x.product_role === 'STRUCTURAL_PARENT'))
        errors.push(`${family.family_asin}: structural parent leaked into members`);
    }
    const dimensionNames = Object.keys(data.dimensions || {});
    if ((data.summary?.amazon_dimension_coverage || 0) > 0 && !dimensionNames.length)
      errors.push('variation metadata exists but dimensional rollups are empty');
    for (const [dimension, rows] of Object.entries(data.dimensions || {}))
      for (const row of rows || []) {
        ...
        if (expected === null ? row.conversion_t28_pct != null : !close(row.conversion_t28_pct, expected))
          errors.push(`${dimension}/${row.value}: dimensional CVR not recomputed`);
      }
    return {
      errors,
      dimensionNames,
...
async function verifyCatalogMode(page, mode) {
  ...
  const semantic = await catalogSemantic(page);
  if (mode.startsWith('dimension:') && !semantic.dimensionNames?.includes(mode.split(':')[1]))
...
async function verifyProductWorkspace(page) {
  ...
  const payload = await page.evaluate(
    async () => await (await fetch('/api/product?sku=PNC-001', { cache: 'no-store' })).json(),
  );
...
async function chooseClosedFinanceMonth(page) {
  ...
  const options = await page.locator('#monthPicker option').evaluateAll((items) =>
    items.map((item) => ({
      value: item.value,
  ...
  if (!closedOption)
    throw new Error('Finance Month view has no closed month available for immutable drill-down QA');
  ...
  if (!closedState || closedState.includes('OPEN'))
    throw new Error(`Finance closed-month drill-down has invalid state: ${closedState || 'blank'}`);
}
...
async function verifyFinanceWindows(page) {
  ...
  for (const windowKey of buttons) {
    ...
    await button.click();
    await page
      .locator(`button[data-finance-window="${windowKey}"][aria-selected="true"]`)
      .waitFor({ state: 'visible', timeout: 5000 });
...
async function verifySalesOrders(page) {
  ...
  const state = await page.evaluate(async () => {
    ...
    return {
      ...
      orderGap: Number.parseFloat(getComputedStyle(document.getElementById('orderRows')).rowGap || '0'),
      ...
      headerHierarchy: rows.every((row) => {
        ...
        return (
          !mobile ||
          (headerStyle.backgroundColor === rowStyle.backgroundColor && headerStyle.boxShadow !== 'none')
        );
      }),
      ...
      namedItems: [...document.querySelectorAll('.sales-order-item strong')].every(
      ...
      itemContracts: rows.map((row, index) => {
        const expected = Array.isArray(apiOrders[index]?.order_items) ? apiOrders[index].order_items : [];
        const rendered = [...row.querySelectorAll('.sales-order-item')];
        const renderedNames = rendered.map((item) => item.querySelector('strong')?.textContent?.trim() || '');
      ...
      statuses: rows.map((row) => row.querySelector('.order-status-pill')?.textContent || ''),
    };
  });
  ...
  if (
    state.mobile &&
    (!state.cardBoundaries || !state.headerHierarchy || state.orderGap < 16 || !state.orderLabels)
  ) {
  ...
  if (brokenItems.length) {
    throw new Error(`Sales Orders item contract mismatch: ${JSON.stringify(brokenItems.slice(0, 5))}`);
  }
      ...
      result.metrics = await page.evaluate(
        ({ viewportName }) => {
          ...
          const textEls = [...document.querySelectorAll('body *')].filter(
            (el) => visible(el) && !el.children.length && (el.textContent || '').trim(),
          );
          const smallText = textEls
            .filter((el) => Number.parseFloat(getComputedStyle(el).fontSize || '0') < minFont)
            .slice(0, 40);
          const clickables = [
          ...
          return {
            ...
            activeTab:
              document
                .querySelector('.tabs button.active,.view-tabs button.active,.analysis-modes button.active')
...
if (summary.navigationFailures || summary.consoleErrorCount || summary.failedResponseCount)
  process.exitCode = 2;
