...
class Settings:
    ...
    # Reporting v3 jobs are asynchronous and can remain queued well beyond fifteen
    # minutes during Amazon-side load. Allow a bounded one-hour wait while keeping
    # the worker's upper bound explicit and operator-configurable.
    ads_report_poll_timeout_seconds: int = int(os.getenv("AMAZON_ADS_REPORT_POLL_TIMEOUT_SECONDS", "3600"))
    ...
    brand_analytics_search_catalog_weekly_backfill_start: str = os.getenv(
        "BRAND_ANALYTICS_SEARCH_CATALOG_WEEKLY_BACKFILL_START", "2025-10-01"
    )
    brand_analytics_search_terms_weekly_backfill_start: str = os.getenv(
        "BRAND_ANALYTICS_SEARCH_TERMS_WEEKLY_BACKFILL_START", "2025-10-01"
    )
